...
include(joinpath(montecarlo_path, "src/spins.jl"))
include(joinpath(montecarlo_path, "src/vectors.jl"))
include(joinpath(montecarlo_path, "src/hamiltonian.jl"))
include(joinpath(montecarlo_path, "src/data.jl"))
include(joinpath(montecarlo_path, "src/metropolis.jl"))
include(joinpath(montecarlo_path, "src/sim.jl"))
include(joinpath(montecarlo_path, "src/plots.jl"))
include(joinpath(montecarlo_path, "src/params.jl"))
...
# B_range = (1.0/(Kb*(T_init*(T/T_init)^(t/T_steps))) for t in 0:T_steps) 
...
@threads for (H,T) in params
    ...
    title = @sprintf("H = %0.3f, T = %0.3f, D = %0.3f", H, T, D)
    writedlm(joinpath(results_path, "spins/HvsT/$(title).txt"), SPINS)
    fig = plot_spins(POS, SPINS, title)
    save(joinpath(results_path, "images/HvsT/$(title).png"), fig)
end
